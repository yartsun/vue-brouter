...
Todos los registros de rutas que hayan coincidido son expuestos en el objeto `$bRoute` (y también a los objetos route en las guardias de navegación) como el array `$bRoute.matched`. Por ende, necesitaremos iterar sobre `$bRoute.matched` para verificar campos meta en los registros de rutas.
