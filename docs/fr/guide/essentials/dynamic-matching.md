...
En plus de `$bRoute.params`, l'objet `$bRoute` expose également d'autres informations utiles comme la `$bRoute.query` (s'il y a une requête dans l'URL), `$bRoute.hash`, etc. Vous pouvez accéder à tous les détails de cela dans la [référence de l'API](../../api/#the-route-object).
